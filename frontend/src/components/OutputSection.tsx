...
const VANTACORE_ATTRIBUTION = 'Generated by VantaCore - Portable Memory Capsules for LLMs.'
const VANTACORE_POSITIONING = 'Compress massive AI sessions into portable memory capsules. 100% client-side.'
const CONTENT_OWNERSHIP_NOTE = 'VantaCore does not claim ownership of your input or output content.'

const FREE_EXPORT_OPTIONS = {
  includeBranding: true,
  proBrandingRemovalAvailable: false,
}

const markdownExport = (result: CompressionResult) => `---
generator: VantaCore
...
const textExport = (result: CompressionResult) => `${VANTACORE_ATTRIBUTION}
${VANTACORE_POSITIONING}
https://vantacore.net
${CONTENT_OWNERSHIP_NOTE}
...
const jsonExport = (result: CompressionResult) => JSON.stringify({
  generator: 'VantaCore',
  generator_url: 'https://vantacore.net',
  format: 'memory-capsule',
  license_note: 'Free export includes attribution.',
  ownership_note: CONTENT_OWNERSHIP_NOTE,
  pro_hooks: {
    branding_removal_available: FREE_EXPORT_OPTIONS.proBrandingRemovalAvailable,
  },
...
const exportContent = (ext: string, result: CompressionResult) => {
  if (!FREE_EXPORT_OPTIONS.includeBranding) return result.compressed
  if (ext === '.json') return jsonExport(result)
  if (ext === '.md') return markdownExport(result)
  return textExport(result)
}
...
export function OutputSection() {
  ...
  const downloadAs = (ext: string) => {
    const content = exportContent(ext, result)
